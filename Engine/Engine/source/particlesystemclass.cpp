...
bool ParticleSystemClass::InitializeParticleSystem()
{
	...
	m_particleDeviationX = 20.0f;
	m_particleDeviationY = 0.1f;
	m_particleDeviationZ = 20.0f;
...
void ParticleSystemClass::EmitParticles(float frameTime)
{
	...
	if ((emitParticle == true) && (m_currentParticleCount < (m_maxParticles - 1)))
	{
		...
		m_particleList[index].red = 1.0f;
		m_particleList[index].green = 1.0f;
		m_particleList[index].blue = 1.0f;
...
void ParticleSystemClass::KillParticles()
{
	...
	for (i = 0; i<m_maxParticles; i++)
	{
		if ((m_particleList[i].active == true) && (m_particleList[i].positionY < -10.0f))			// -3.0 original
		{
			m_particleList[i].active = false;
			m_currentParticleCount--;

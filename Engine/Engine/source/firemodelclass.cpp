...
bool FireModelClass::Initialize(ID3D11Device* device, char* modelFilename, WCHAR* textureFilename1, WCHAR* textureFilename2, WCHAR* textureFilename3)
{
	bool result;
...
bool FireModelClass::LoadTextures(ID3D11Device* device, WCHAR* textureFilename1, WCHAR* textureFilename2, WCHAR* textureFilename3)
{
	...
	for (int i = 0; i < 3; i++)
	{
		m_Texture[i] = new TextureClass();
		if (!m_Texture[i])
			return false;
	}
	result = m_Texture[0]->Initialize(device, textureFilename1);
	if (!result)
		return false;
	result = m_Texture[1]->Initialize(device, textureFilename2);
	if (!result)
		return false;
	result = m_Texture[2]->Initialize(device, textureFilename3);
	if (!result)
		return false;
...
{
	return m_Texture[1]->GetTexture();
}

ID3D11ShaderResourceView* FireModelClass::GetTexture3()
{
	return m_Texture[2]->GetTexture();
}
